...
export default function ExpenseList({ route, navigation }) {
    ...
    if (userId) {
      try {
        ...
        if (selectedFilter === "Khoảng tiền") {
          const amountFrom = minAmount
            ? parseInt(minAmount.replace(/,/g, ""))
            : 0;
          const amountTo = maxAmount
            ? parseInt(maxAmount.replace(/,/g, ""))
            : Infinity;
          filteredTransactions = filteredTransactions.filter((transaction) => {
            const amount = Math.abs(transaction.amount);
            return (
              (!amountFrom || amount >= amountFrom) &&
        ...
        if (selectedFilter === "Khoảng thời gian") {
          ...
          filteredTransactions = filteredTransactions.filter((transaction) => {
            ...
            return (
              transactionDate >= startOfStartDate &&
              transactionDate <= endOfEndDate
            );
          });
        }
  ...
  return (
    <ScrollView style={tw`flex-1`} contentContainerStyle={tw`p-2`}>
      ...
      >
        <View
          style={tw`flex-1 justify-end items-center bg-opacity-50 bg-black`}
        >
          <View style={tw`bg-white rounded-t-3xl w-full p-6 shadow-lg`}>
            ...
            >
              <Text style={tw`text-black text-center font-semibold`}>Đóng</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
