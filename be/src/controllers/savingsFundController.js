...
class SavingsFundController {
    ...
    static async editSavingsFund(req, res) {
        try {
            ...
            if (categoryId !== undefined) fund.categoryId = categoryId;
            if (startDate !== undefined) fund.startDate = new Date(startDate);
            if (endDate !== undefined) fund.endDate = new Date(endDate);
    
            // Save the updated savings fund document
            await fund.save();
    
            // Respond with the updated savings fund
            res.status(200).json({ message: 'Cập nhật quỹ tiết kiệm thành công', data: fund });
        } catch (error) {
