...
const sendConfirmationEmail = async (email, code) => {
    ...
    const mailOptions = {
        ...
        <div style="font-family: 'Roboto', Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; border-radius: 10px; background-color: #ffffff; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1);">
            ...
            <!-- Title -->
            <h2 style="color: rgb(37, 130, 223); text-align: center; font-size: 20px; margin-bottom: 15px; font-weight: 600;">
            ...
            <!-- Confirmation Code -->
            <div style="color:rgb(37, 130, 223); font-size: 24px; font-weight: bold; text-align: center; padding: 15px; margin-bottom: 20px; border: 2px dashed rgb(129, 167, 199); border-radius: 6px;">
                ${code}
            </div>
            ...
            <footer style="text-align: center; font-size: 12px; color: #999;">
                <p>BeeMoney - Giải pháp tài chính thông minh.</p></footer>
        </div>
        `
    };
...
class SavingsFundController {
    ...
    static async acceptInviteByCode(req, res) {
        ...
        try {
            ...
            if (savingsFund.members.some(member => member.userId.equals(req.user.id))) {return res.status(400).json({ message: 'Tài khoản đã tồn tại trong quỹ tiết kiệm' });
        }
